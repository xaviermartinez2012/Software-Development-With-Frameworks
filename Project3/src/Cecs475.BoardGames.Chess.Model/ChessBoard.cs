...
namespace Cecs475.BoardGames.Chess.Model {
    ...
    public class ChessBoard : IGameBoard {
        ...
        private List<ChessMove> GetHorizontalVerticalAttackMoves(BoardPosition pos)
        {
            var attackMoves = new List<ChessMove>();
            int row = pos.Row;
            int col = pos.Col;
        ...
        private List<ChessMove> GetDiagonalAttackMoves(BoardPosition pos)
        {
            var validAttackMoves = new List<ChessMove>();
            int row = pos.Row;
            int col = pos.Col;
            ...
            get
            {
                ...
                foreach (var pos in whitePawns)
                {
                    ...
                    var joinAttackKnight = from move in validAttackMoves
                                           ...
                                           select iPos;
                    var joinAttackBishop = from move in validAttackMoves
                                           join iPos in blackBishops on move.EndPosition equals iPos
                                           select iPos;
                    var joinAttackRook = from move in validAttackMoves
                                           join iPos in blackRooks on move.EndPosition equals iPos
                                           select iPos;
                    var joinAttackQueen = from move in validAttackMoves
                                          join iPos in blackQueen on move.EndPosition equals iPos
                                          select iPos;
                    var joinAttackKing = from move in validAttackMoves
                                         join iPos in blackKing on move.EndPosition equals iPos
                                         select iPos;
                ...
                foreach (var pos in blackPawns)
                {
                    ...
                    var joinAttackBishop = from move in validAttackMoves
                                           ...
                                           select iPos;
                    var joinAttackRook = from move in validAttackMoves
                                         join iPos in whiteRooks on move.EndPosition equals iPos
                                         select iPos;
                    var joinAttackQueen = from move in validAttackMoves
                                          join iPos in whiteQueen on move.EndPosition equals iPos
                                          select iPos;
                    var joinAttackKing = from move in validAttackMoves
                                         join iPos in whiteKing on move.EndPosition equals iPos
                                         select iPos;
                    blackWeight += joinBishop.Count();
